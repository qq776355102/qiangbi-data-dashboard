...
export default function App() {
    ...
    const handleRefresh = async () => {
        ...
        try {
            ...
            alert(`Refresh failed: ${error.message || "Unknown error"}. Please check your RPC URL.`);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-[1680px] mx-auto px-6 py-8">
            <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 font-medium">Tracking rewards and assets across multiple modules</p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 w-full xl:w-auto">
                    <div className="flex items-center gap-3 bg-slate-800/80 backdrop-blur-sm p-2.5 rounded-xl border border-slate-700 shadow-inner">
                        ...
                        <span className="text-xs font-black text-slate-400 uppercase">Batch:</span>
                        <select 
                            value={batchSize}
                            onChange={(e) => setBatchSize(Number(e.target.value))}
                            className="bg-slate-900 text-indigo-400 text-xs font-black px-3 py-1 rounded-lg border border-slate-700 outline-none"
                        >
                    ...
                    <div className="relative flex-1 xl:flex-none">
                        <input 
                            ...
                            className="bg-slate-800 border border-slate-700 text-slate-200 rounded-xl px-4 py-3 text-sm w-full xl:w-96 outline-none shadow-inner"
                            placeholder="RPC URL"
                        />
                    </div>
                    <button 
                        ...
                        className={`flex items-center justify-center gap-2 px-8 py-3 rounded-xl font-black text-sm transition-all shadow-2xl ${
                            loading ? 'bg-slate-700 text-slate-400' : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-600/40 hover:-translate-y-0.5'
                        }`}
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4 mb-10">
                ...
                <StatCard icon={<Coins className="text-pink-400" />} label="LGNS" value={globalStats.dlgns} color="pink" />
                <StatCard icon={<Layers className="text-cyan-400" />} label="slgns" value={globalStats.dslgns} color="cyan" />
            </div>
            ...
            <div className="bg-slate-800/50 border border-slate-700 p-6 rounded-3xl flex flex-col md:flex-row justify-between items-center gap-6 mb-8 shadow-2xl">
                <div className="relative w-full md:w-[600px]">
                    ...
                    <input 
                        ...
                        className="bg-slate-900 border border-slate-700 text-slate-100 rounded-2xl pl-12 pr-4 py-4 w-full outline-none shadow-inner text-lg"
                    />
                </div>
                <div className="flex items-center gap-4 w-full md:w-auto">
                    <label className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-slate-700 hover:bg-slate-600 text-slate-100 rounded-2xl cursor-pointer font-bold border border-slate-600/50">
                        <Upload className="w-5 h-5" />
                        Import JSON
                        <input type="file" className="hidden" accept=".json" onChange={handleImport} />
                    </label>
                    <button onClick={handleExport} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-slate-700 hover:bg-slate-600 text-slate-100 rounded-2xl font-bold border border-slate-600/50">
            ...
            <div className="bg-slate-800/80 backdrop-blur-md border border-slate-700 rounded-[2rem] overflow-hidden shadow-2xl">
                    ...
                    <table className="w-full text-left border-collapse min-w-[1300px]">
                        <thead>
                            <tr className="bg-slate-900/80 border-b border-slate-700 text-[11px] font-black text-slate-500 uppercase tracking-widest">
                                ...
                                <th className="px-6 py-6 text-right">Rewards</th>
                                <th className="px-6 py-6">Derived Assets</th>
                                <th className="px-8 py-6 text-center">Log</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-700/20">
                            {filteredAddresses.map((addr) => {
                                ...
                                return (
                                    <tr key={addr.aAddress} className="group hover:bg-slate-700/30 transition-all">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    {editingRemark?.address === addr.aAddress ? (
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            />
                                                            <button onClick={() => handleUpdateRemark(addr.aAddress, editingRemark.value)} className="bg-emerald-500 p-2 rounded-lg"><Save className="w-4 h-4" /></button>
                                                        </div>
                                                    ) : (
                                                        <div className="flex items-center gap-3">
                                                            <span className="font-black text-lg text-slate-200">{addr.remark || 'N/A'}</span>
                                                            <button onClick={() => setEditingRemark({ address: addr.aAddress, value: addr.remark })} className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 bg-slate-700 rounded-lg text-slate-400">
                                                                <Edit2 className="w-3.5 h-3.5" />
                                                            </button>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="flex items-center gap-3 bg-slate-900/60 rounded-xl px-4 py-3 w-fit border border-slate-700/50">
                                                    <span className="text-xs font-mono text-slate-400 font-bold break-all">{addr.aAddress}</span>
                                                    <button onClick={() => copyToClipboard(addr.aAddress)} className="text-slate-600 hover:text-indigo-400">
                                        ...
                                        <td className="px-6 py-8 text-right">
                                            <div className="flex flex-col gap-2">
                                                <div className="text-[11px] text-blue-400 font-black bg-blue-400/10 px-3 py-1.5 rounded-xl border border-blue-400/20 inline-block ml-auto">MINT: {formatNum(data.mintStaking)}</div>
                                                <div className="text-[11px] text-purple-400 font-black bg-purple-400/10 px-3 py-1.5 rounded-xl border border-purple-400/20 inline-block ml-auto">BOND: {formatNum(data.bondStaking)}</div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-8 text-right">
                                        ...
                                        <td className="px-6 py-8 text-right">
                                            <div className="flex flex-col items-end gap-2">
                                                <span className="text-sm font-black text-emerald-400">TURB: {formatNum(data.turbineBalance)}</span>
                                                <span className="text-sm font-black text-amber-400">SPID: {formatNum(data.spiderWebReward)}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-8">
                                            <div className="flex flex-col gap-2 w-[160px]">
                                                <div className="flex justify-between items-center px-3 py-2 bg-slate-900/90 rounded-xl border border-slate-700/50">
                                                    <span className="text-[10px] text-slate-500 font-black">LGNS</span>
                                                    <span className="text-pink-400 font-black text-xs">{formatNum(data.derivedLgns)}</span>
                                                </div>
                                                <div className="flex justify-between items-center px-3 py-2 bg-slate-900/90 rounded-xl border border-slate-700/50">
                                                    <span className="text-[10px] text-slate-500 font-black">slgns</span>
                                        ...
                                        <td className="px-8 py-8 text-center">
                                            <div className="flex flex-col gap-2 items-center">
                                                <span className="text-[11px] font-black bg-slate-900 border border-slate-700 px-4 py-2 rounded-xl text-slate-500 min-w-[100px]">
                                                    {addr.log}
                                                </span>
                                            </div>
                                        </td>
                ...
                {filteredAddresses.length === 0 && (
                    <div className="p-32 text-center bg-slate-900/20">
                        ...
                        <p className="text-slate-500 font-black text-2xl italic tracking-tight">No accounts match the search criteria.</p>
                        <button onClick={() => setSearchTerm('')} className="mt-6 px-8 py-3 bg-slate-800 text-indigo-400 rounded-2xl font-black">Clear Search</button>
                    </div>
                )}
            </div>
        </div>
...
function StatCard({ icon, label, value, color }: { icon: React.ReactNode, label: string, value: number, color: string }) {
    return (
        <div className={`bg-slate-800/40 border-2 border-slate-700/40 p-5 rounded-3xl hover:border-${color}-500/50 transition-all group overflow-hidden`}>
            <div className="flex items-center gap-3 mb-4 relative z-10">
                <div className={`p-2.5 bg-slate-900 rounded-2xl shadow-inner border border-slate-700/50`}>{icon}</div>
                <span className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 truncate">{label}</span>
            </div>
            <div className="text-2xl font-black text-white truncate px-1">
                {value.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                <span className="text-[9px] ml-2 text-slate-700 font-black">Units</span>
            </div>
        </div>
    );
}
